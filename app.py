...
import json
from cracker import calculate_shannon_entropy, seconds_to_human_readable
...
def check_strength():
    ...
    # 1. Calculate Entropy (Using your updated standard formula)
    entropy = calculate_shannon_entropy(password)

    # 2. Estimate Crack Time (Assume high-end cracking: 10^10 guesses/sec)
    guesses_per_second = 10**10 # 10 billion guesses/sec
    total_combinations = 2**entropy
    time_in_seconds = total_combinations / guesses_per_second

    # 3. Get Human-Readable Time (Using the new detailed function)
    crack_time_str = seconds_to_human_readable(time_in_seconds)

    # 4. Determine Strength Description based on time (More granular)
    strength = "Very Weak"
    # Thresholds based loosely on common time estimates
    if time_in_seconds >= 60 * 60 * 24 * 365 * 100: # 100+ years
        strength = "Very Strong"
    elif time_in_seconds >= 60 * 60 * 24 * 30: # 1+ month
        strength = "Strong"
    elif time_in_seconds >= 60 * 60 * 8: # 8+ hours
        strength = "Moderate"
    elif time_in_seconds >= 1: # 1+ second
        strength = "Weak"
    # else remains "Very Weak" (less than a second)

    return jsonify({
        'strength': strength,
        'crack_time': crack_time_str,
        'entropy_bits': round(entropy, 2) # You can still return this if desired
    })

@app.route('/strength_checker')
def strength_checker_page():
    """Renders the dedicated password strength checker page."""
